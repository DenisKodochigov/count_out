...
class RunWorkOut @Inject constructor(
    private val speechManager: SpeechManager, val context: Context) {
...
//    val runningCountRest = Object()
    ...
    suspend fun runWorkOut(dataForWork: DataForWork, dataFromWork: DataFromWork){
        ...
        dataForWork.training.value?.let { tr->
            tr.speech?.let {
                it.beforeStart?.addMessage = tr.name
                speechStart(dataFromWork, it) }
            dataForWork.map.forEachIndexed { indM, item->
                ...
                if ( currentIndExercise != item.exercise?.idExercise ) {
                    ...
                    item.exercise?.let { exerciseLet->
                        exerciseLet.speech?.let { speechStart(dataFromWork, it) }
                        val desc = if (dataForWork.enableSpeechDescription.value) {
                            exerciseLet.activity?.description ?: ""
                        } else ""
                        speechManager.speech(dataFromWork,
                            SpeechImpl( message = "${context.getString(R.string.next_exercise)} " +
...
//                    dataForWork.setExecuteInfoExercise(index = indM)
            ...
            tr.speech?.let { speechEnd(dataFromWork, it) }
...
//        runningCountRest.wait()
    ...
    private suspend fun speakSetBegin(set: Set, dataFromWork: DataFromWork){
        speechManager.speech(dataFromWork, SpeechImpl(message = textBeforeSet(set)))
        set.speech?.let { speechStart(dataFromWork, it) }
    }
    ...
    private suspend fun speakingDURATION(set: Set, dataFromWork: DataFromWork){
        speakInterval( duration = (set.duration.value / (if (set.duration.unit == Units.M) 60 else 1)).toInt(),
    ...
    private suspend fun speakEnd(setCurrent: Set, dataFromWork: DataFromWork){
        setCurrent.speech?.let { speechEnd(dataFromWork, it) }
        ...
        speechManager.speech(dataFromWork, SpeechImpl(
            message = getStr(R.string.rest) + "  " +
                    getPlurals(
                        (setCurrent.rest.value / (if (setCurrent.rest.unit == Units.M) 60 else 1)).toInt(),
    ...
    private fun textBeforeSet(set: Set): String{
        return when (set.goal){
            Goal.Count -> " " + getPlurals(set.reps, R.plurals.repeat)
            Goal.CountGroup -> " " + getPlurals(set.reps, R.plurals.repeat)
            Goal.Duration ->
                getPlurals((set.duration.value / (if (set.duration.unit == Units.M) 60 else 1)).toInt(),
                    set.duration.unit.id)
            Goal.Distance ->
                getPlurals((set.duration.value / (if (set.distance.unit == Units.KM) 1000 else 1)).toInt(),
                    set.distance.unit.id)
        }
    }
