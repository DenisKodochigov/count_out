...
fun StartDragDrop()
{
    ...
    fun onItemClicked(clickedItem: Item) {
        itemsStateFlow.update { currentList ->
            val newList = currentList.toMutableList().map { item ->
                if(clickedItem == item) { item.copy(name = "Clicked ${item.name}")
                } else { item }
            }.toList()
            newList
        }
...
fun DragNDropItemsList(
    ...
    val mutableUiState = MutableStateFlow(items)
    ...
    DragDropColumn(items = uiState.value, onSwap = ::swapItems
    ) { item ->
        Card(modifier = Modifier.clickable { onItemClicked(item) },
        ){
            Text(
                text = item.name,
...
fun DefaultPreview() {
    AppTheme {
        ...
        DragNDropItemsList(items = mutableUiState.collectAsState().value)
    }
}