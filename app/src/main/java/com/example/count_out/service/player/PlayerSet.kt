...
class PlayerSet @Inject constructor(val speechManager:SpeechManager)
{
    ...
    suspend fun playingSet(set: Set, stateService: (StateWorkOut)->Unit){
        ...
        speechManager.speech(set.speech.beforeStart + textBeforeSet, stateService)
        speechManager.speech(set.speech.afterStart, stateService)
        ...
        if (set.timeRest > 0) {
            speechManager.speech(set.timeRest.toString() + " секунд отдыха.", stateService)
            delay((set.timeRest * 1000).toLong())
        }
        speechManager.speech(set.speech.beforeEnd, stateService)
        speechManager.speech(set.speech.afterEnd, stateService)
    }
}