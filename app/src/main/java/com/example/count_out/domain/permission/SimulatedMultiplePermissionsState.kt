...
sealed interface LocationPermissionsState {
    @Composable operator fun invoke(): MultiplePermissionsState
...
private suspend fun requestToEnableGPS(context: Context, snackbarHostState: SnackbarHostState) {
    ...
    val canNavigateToGPSSettings = intent.resolveActivity(context.packageManager) != null

    val result = snackbarHostState.showSnackbar(
        message = "GPS is disabled",
        actionLabel = if (!canNavigateToGPSSettings) { null } else { "ENABLE" },
    ...
    when (result) {
        ...
        SnackbarResult.ActionPerformed -> { if (canNavigateToGPSSettings) { context.startActivity(intent) } }
    }
}
    ...
    DisposableEffect(
        ...
        if (!permissions.allPermissionsGranted || permissions.shouldShowRationale) {
            scope.launch {
                ...
                when (result) {
                    SnackbarResult.Dismissed -> {}
                    SnackbarResult.ActionPerformed -> { permissions.launchMultiplePermissionRequest() }
                }
            }
