...
@Composable
fun RequestPermissionsAll( activity: Activity){
    ...
    checkPermissions(permissions1, activity)
    checkPermissions(permissions2, activity)
...
fun checkPermissions(permissions: List<String>, activity: Activity){
    permissions.forEach { permis->
        val hasPermission = activity.let {
            ContextCompat.checkSelfPermission(it, permis) == PackageManager.PERMISSION_GRANTED }
        if (! hasPermission) lg("RequestPermissions permissions not granted $permis")
    }
}
