...
class ActivitySourceImpl @Inject constructor(private val dao: ActivityDao): ActivitySource {
    override fun gets(): Flow<List<ActivityImpl>> = dao.gets()
        .map { list-> list.map { it.toActivity() }}.catch { ThrowableUC.TrainingThrow(it) }

    override fun get(id: Long): Flow<ActivityImpl> = dao.get(id)
        .map { it.toActivity() }.catch { ThrowableUC.TrainingThrow(it) }
