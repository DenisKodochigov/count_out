...
class ExerciseSourceImpl @Inject constructor(
    ...
    override fun gets(): Flow<List<com.count_out.domain.entity.workout.Exercise>> = dao.gets().map { list-> list.map { it.toExercise() } }
    ...
    override fun add(roundId: Long, ringId: Long): Flow<List<ExerciseImpl>> {
        val result  = if (roundId > 0L || ringId > 0L) {
            val idExercise = dao.add(
                ExerciseTable(speechId = com.count_out.domain.entity.workout.SpeechKit.idSpeechKit))
    ...
    private fun newSetImpl(exerciseId: Long) = SetImpl(
        ...
        speechId = 0,
        speech = null,
        goal = com.count_out.domain.entity.enums.Goal.Count,
        weight = ParameterImpl(value = 1.0, unit = com.count_out.domain.entity.enums.Units.KG),
        distance = ParameterImpl(value = 1.0, unit = com.count_out.domain.entity.enums.Units.MT),
        duration = ParameterImpl(value = 1.0, unit = com.count_out.domain.entity.enums.Units.S),
        reps = 10,
        intensity = com.count_out.domain.entity.enums.Zone.Medium,
        intervalReps = 1.0,
        intervalDown = 0,
        groupCount = "",
        rest = ParameterImpl(value = 1.0, unit = com.count_out.domain.entity.enums.Units.S),
    )
}